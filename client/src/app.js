...
(function() {
    ...
    function renderQuotesTable() {
        ...
        tbody.innerHTML = AppState.quotes.map(quote => `
            <tr>
                ...
                <td class="text-muted">${formatDate(quote.createdAt)}</td>
                <td>
    ...
    window.viewQuote = async function(id) {
        try {
            ...
            modal.querySelector('#detail-created-date').textContent = formatDate(quote.createdAt);
        ...
        if (authorFilter) {
            authorFilter.innerHTML = '<option value="">Tous les auteurs</option>' +
                AppState.authors.map(author => 
                    `<option value="${escapeHtml(author.name)}">${escapeHtml(author.name)} (${author.quotesCount})</option>`
                ).join('');
        }
        ...
        if (tagFilter) {
            tagFilter.innerHTML = '<option value="">Tous les tags</option>' +
                AppState.tags.map(tag => 
                    `<option value="${escapeHtml(tag.name)}">${escapeHtml(tag.name)} (${tag.usageCount})</option>`
                ).join('');
        }
    }
